  ...
  useEffect(() => {
    async function fetchLatest() {
      ...
      const [agroArr, mentorArr, tutArr, artArr] = await Promise.all([
        fetch("http://localhost:5000/agrotourism", { headers }).then((res) =>
          res.json()
        ),
        fetch("http://localhost:5000/mentorship", { headers }).then((res) =>
          res.json()
        ),
        fetch("http://localhost:5000/tutorials", { headers }).then((res) =>
          res.json()
        ),
        fetch("http://localhost:5000/articles", { headers }).then((res) =>
          res.json()
        ),
      ]);
      ...
      const getLatest = (arr, idKey = "id") => {
        ...
        return arr.reduce(
          (max, item) =>
            item[idKey] > (max?.[idKey] ?? -Infinity) ? item : max,
          arr[0]
        );
      };
      setLatest({
            ...
            <div style={{ width: "100%", height: 300, maxWidth: 1200 }}>
              <ResponsiveContainer width="100%" height={300}>
                ...
                >
                  ...
                  <XAxis
                    dataKey="name"
                    tick={{ fontWeight: 700, fontSize: 16, fill: "#22313F" }}
                    axisLine={{ stroke: "#888", strokeWidth: 2 }}
                    tickLine={false}
                  />
                  <YAxis
                    tick={{ fontWeight: 700, fontSize: 15, fill: "#444" }}
                    axisLine={{ stroke: "#888", strokeWidth: 2 }}
                    tickLine={false}
                    width={50}
                    domain={[0, 100]}
                  ...
                  />
                  <Tooltip
                    cursor={{ fill: "rgba(140,196,127,0.12)" }}
                    contentStyle={{
                      borderRadius: 8,
                      fontWeight: 700,
                      fontSize: 16,
                      color: "#22313F",
                      border: "1.5px solid #8BC47F",
                    }}
                    formatter={(v) => [`${v}k`, "Price"]}
                  />
                  <Bar
                    dataKey="value"
                    fill="#8BC47F"
        ...
        >
          ...
          <div style={cardStyle}>
            {latest.agrotourism ? (
              <>
                <img
                  src={latest.agrotourism.image_url}
                  alt={latest.agrotourism.name}
                  style={imgStyle}
                />
                <div style={titleStyle}>{latest.agrotourism.name}</div>
                <div style={descStyle}>
                  {latest.agrotourism.city}, {latest.agrotourism.province}
                </div>
                <button
                  onClick={() =>
                    navigate(`/agrotourism/${latest.agrotourism.id_agrowisata}`)
                  }
                  style={btnStyle}
                >
                  See Detail
          ...
          <div style={cardStyle}>
            {latest.mentorship ? (
              <>
                <img
                  src={latest.mentorship.image_url}
                  alt={latest.mentorship.title}
                  style={imgStyle}
                />
                <div style={titleStyle}>{latest.mentorship.title}</div>
                <div style={descStyle}>
                  {latest.mentorship.description &&
                  latest.mentorship.description.length > 100 ? (
                    <>
                      ...
                      <span
                        ...
                        onClick={() =>
                          navigate(
                            `/mentorship/${latest.mentorship.id_mentorship}`
                          )
                        }
                      >
                        See more
                ...
                <button
                  onClick={() =>
                    navigate(`/mentorship/${latest.mentorship.id_mentorship}`)
                  }
                  style={btnStyle}
                >
                  See Detail
